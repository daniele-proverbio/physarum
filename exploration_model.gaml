...
global torus: torus_environment
{
	...
	reflex score_check when: cycle > stop_cycle - 1
	{
		...
		//if cycle = stop_cycle { write max(ten_scores); }
	}
    ...
    init
    {
        ...
        {
             geometry spawn <- circle(gd/20,{gd/2,gd/2});             
	...
	string pic_destination <- base + "/EM/" + gr + "/";
	string score_destination <- base + "/EM/sal/" + gr + "/" + sensor_arm_length;
	...
	{
		save score to: score_destination + "_pert.csv" format: "csv" header: false rewrite: false;
	}
...
species physarum skills: [moving] control: fsm schedules: shuffle(physarum)
{
    ...
    reflex is_there_food  //check whether there is food
	{
		...
		if hunger - luck > starvation_threshold
		{
			iAmStarving <- true;
		}
	}
...
experiment performance type: batch repeat: 100 keep_seed: false until: (cycle = stop_cycle)  {
	int samples <- 14;
	int steps <- 1;
